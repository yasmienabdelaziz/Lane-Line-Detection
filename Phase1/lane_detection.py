...
if (mode == 0):
    myclip = VideoFileClip(videopath)  # .subclip(40,43)
    output_vid = output_location
    ...
    while(cap.isOpened()):
        ...
        windows, curves, lanes, ploty = sliding_window(
            BirdEyeHLS, draw_windows=True)

        BirdEyeHLS = cv2.cvtColor(BirdEyeHLS, cv2.COLOR_GRAY2RGB)
        HLS = cv2.cvtColor(HLS, cv2.COLOR_GRAY2RGB)
        BirdEyeHLS = cv2.resize(
            BirdEyeHLS, (640, 360), interpolation=cv2.INTER_AREA)
        HLS = cv2.resize(HLS, (640, 360), interpolation=cv2.INTER_AREA)
        BirdEye = cv2.resize(BirdEye, (640, 360),
                             interpolation=cv2.INTER_AREA)
        windows = cv2.resize(windows, (640, 360),
                             interpolation=cv2.INTER_AREA)
        out_frame = cv2.resize(out_frame, (640, 360),
                               interpolation=cv2.INTER_AREA)
        pure_frame = cv2.resize(
            pure_frame, (640, 360), interpolation=cv2.INTER_AREA)
        BirdEyeHLS = cv2.normalize(
            BirdEyeHLS, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
